...
targetsFile="files/targets_zrdn3.txt"
ammunitionFile="temp/ZRDN3_Missiles"
echo "" > $targetsFile
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        then
                            echo -e "\e[1;36m __ZRDN_3__ Обнаружена цель Самолет ID:$id с координатами $X $Y"
                            echo -e "\e[1;36m __ZRDN_3__ Shoots at the target ID:$id "
                            touch "$pathD/$id"
                            echo "shoot" >> $ammunitionFile
                            L=`cat $ammunitionFile | wc -l`
                            Missilesremained=`echo "$NumOfMissiles - $L" | bc`
                            echo -e "\e[1;36m __ZRDN_3__ $Missilesremained Missiles remained"
                            if [[ $Missilesremained -eq 0 ]]
                            then 
                                echo -e "\e[1;36m __ZRDN_3__ Ammunition is empty"
                                > $ammunitionFile
                                echo -e "\e[1;36m __ZRDN_3__ Ammunition replenished"
                            fi
                        else
                            if [[ ($Distance -ge $KRSpeedL) && ($Distance -le $KRSpeedH) ]]
                            then
                                echo -e "\e[1;36m __ZRDN_3__ Обнаружена цель К.ракета ID:$id с координатами $X $Y"
                                echo -e "\e[1;36m __ZRDN_3__ Shoots at the target ID:$id "
                                touch "$pathD/$id"
                                echo "shoot" >> $ammunitionFile
                                L=`cat $ammunitionFile | wc -l`
